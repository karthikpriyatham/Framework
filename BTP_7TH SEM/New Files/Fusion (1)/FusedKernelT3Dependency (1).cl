...
,
 ...
 // float3
		...
		//3
				const float4 invK_data_0,
	//4
				const float4 invK_data_1,
	//5
				const float4 invK_data_2,
	//6
...

	{
			 ...
			 int X = FVALUE*BLOCK_SIZE*BLOCK_SIZE+OFFSET;

		 if(((WORKGROUP_SIZE/BLOCK_SIZE)*group_y + group_x <X))
		{
	...
	barrier(CLK_LOCAL_MEM_FENCE);
		 ...
		 if(!((WORKGROUP_SIZE/BLOCK_SIZE)*group_y + group_x <X))+'
		{
			...
			if(pixel.x >= depthSize.x || pixel.y >= Size.y ) {
			...
			if(out[pixel.x + Size.x * pixel.y] > 0) {

				res = out[pixel.x + Size.x * pixel.y] * (myrotate(invK_data_0,invK_data_1,invK_data_2,invK_data_3, (float3)(pixel.x, pixel.y, 1.f)));
