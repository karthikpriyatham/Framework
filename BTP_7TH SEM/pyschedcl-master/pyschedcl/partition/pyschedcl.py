                        ...
                        for files in range(kernel.buffer_info['output']) :
                            ...
                            if 'loc' in kernel.buffer_info['output'][files] :
                                ...
                                if 'jpeg' or 'jpg' in files['loc']:
                                    ...
                                    filename = "inputs/"+kernel.buffer_info['output'][files]['loc']
                                    img.save(filename)
                                    im.show()
...
class Kernel(object):
        ...
        for btype in ['input', 'output', 'io']:
            for i in range(len(self.buffer_info[btype])):
                ...
                if 'loc' in self.buffer_info[btype][i] :
                    ...
                    self.filenames[btype].append("random")
    ...
    def load_data_from_file(self):
        ...
        for btype in [0,1] :
            ...
            for idx in range(0,self.filenames[types[btype]]):

                file = self.filenames[types[btype]][idx]
                if "random" in file :
                    continue

                if ".txt" not in file  :

                    im = Image.open("inputs/"+file) #Can be many different formats.
                    
                
                    pixels = im.load()  #might change the directory location
                    for j in range(0,pixels.size[0]) :
                        for k in range(0,pixels.size[1]):
                            buffer_data[btype][idx].append(int(pixels[j][k]))
                ...
                for j in range(0,dim1):    
                    ...
                    for k in range(0,prod):
                        buffer_data[btype][idx].append(int(data_in_linej[k]))
        ...
        self.data=[]
        for btype in [0,1,2]:
            self.data[types[btype]] = list(buffer_data[btype])
