            ...
            if event_type == 'WRITE':
                ...
                if kernel.chunks_left == 0:
                    ...
                    if dump_output:
                        ...
                        for files in range(kernel.buffer_info['output']) :

                            if 'loc' in kernel.buffer_info['output'][files] :

                                if 'jpeg' or 'jpg' in files['loc']:
                                    sizes = kernel.output_input_dump_relation[files]
                                    b = np.reshape(kernel.data['output'][files],(sizes[0],sizes[1]))
                                    img = Image.fromarray(b, 'RGB')
                                    filename = SOURCE_DIR + "output/" + kernel.name + '_'+ str(kernel.partition)+ '_' + str(files)
                                    img.save(filename)
                                    im.show()
                                ...
                                with open(filename, flags[ite]) as handle:
...
class Kernel(object):
    ...
    def __init__(self, src, dataset=1024, partition=None, identifier=None):
        ...
        #add inputfilenames to this

        self.i_o = False
        self.filenames = dict() 
        if 'inputfile' in src :
            self.filenames['input']  = [] + src['inputfile']
            self.i_o = True
            dump_output= True
        
        if 'outputfile' in src :
            self.filenames['output'] = [] + src['outputfile']
            self.i_o = True
            dump_output= True

        if 'iofile' in src :
            self.filenames['io'] = [] + src['iofile']
            self.i_o = True
            dump_output= True
        ...
        #this will map output to input for image files
        self.output_input_dump_relation = dict()
        ...
        for btype in ['input', 'output', 'io']:
            for i in range(len(self.buffer_info[btype])):
                ...
                if 'loc' in self.buffer_info[btype][i] :
                    #implies it is an image type file
                    

                    self.i_o = True
                    self.filenames[btype].append(self.buffer_info[btype][i]['loc'])
                    dump_output= True

                    if 'jpeg' or 'jpg' in self.buffer_info[btype][i]['loc'] :


                        if btype =='input' :
                            im = Image.open(self.buffer_info[btype][i]['loc']) #Can be many different formats.
                            pixels = im.load(self.buffer_info[btype][i]['loc'])
                            self.output_input_dump_relation[self.buffer_info[btype][i]['dump_to']] = [pixels.size[0],pixels.size[1]]
    ...
    def load_data_from_file(self):
        ...
        """
            ...
            Formats considered are jpeg and txt

            If we reach a case , where a kernel has an image and some other input obtained from a text file,
            we need to handle both , so we keep self.input_buffers as a list 


            The format of that file will be 

                    buffertype # buftype wont be used i think
                    totaldimensions dimension1 dimension2  .... # till last dimension followed by dim1*dim2*dim3* .. numbers4
                    a(0,0,0,0) a(0,0,0,1) ..... and so on  followed by space 
        ...
        buffer_data = []
        buffer_data.append([])    #for input , io ,output
        buffer_data.append([])
        buffer_data.append([])
        
        types = ['input','io','output']

        for btype in [0,1,2]:
            for i in range(0,len(self.buffer_info[types[btype]])):
                buffer_data[btype].append([])


        #we will handle only inputfiles for now
        for file in self.filenames['input']:
                ...
                if ".txt" not in file  :
                    ...
                    pixels = im.load("/src/"+file)
                    for j in range(0,pixels.size[0]) :
                        for k in range(0,pixels.size[1]):
                            buffer_data[0][pos].append(int(pixels[j][k]))
                ...
                for i in range(N):
                    ...
                    bufinfo = data[line].split()
                    buftype = int(bufinfo[0]) # buffer type
                    bufpos = int(bufinfo[1]) # buffer pos 

                    line = line +1
                    if bufname in self.data :

                    
                        ndim_s = data[line].split()
                        line = line +1

                        prod = 1
                        for dim in range(2,len(ndim_s)) :
                            prod = prod * int(ndim_s[dim])

                        #now iterate dimension1 lines ,each containing prod number of inputs
                        dim1 = ndim_s[1]

                        for j in range(0,dim1):
                        
                            data_in_linej = data[line+j].split()
                            for k in range(0,prod):
                                buffer_data[buftype][bufpos].append(int(data_in_linej[k]))

                        line = line + dim1  #feed forward to the next buffer

                    else : 

                        #ignore this buffer and go to the next one
                        ndim_s = data[line].split()
                        line = line + int(ndim_s[1])+1


        for btype in [0,1,2]:
            for i in range(0,len(self.buffer_info[types[btype]])):
                if not buffer_data[btype][i] :
                    buffer_data[ btype][i] = [] + np.zeros(self.buffer_info[types[btype]][i]['size'], dtype=ctype(self.buffer_info[types[btype]][i]['type']),
                         order='C')


        #copy buffer data to self.data
        self.data = list(buffer_data)
